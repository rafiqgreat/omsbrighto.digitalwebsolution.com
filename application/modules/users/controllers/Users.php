	...
	public function datatable_json(){				   					   
		...
		{  
			...
			$data[]= array(
				...
				'<a title="View" class="view btn btn-xs btn-info" href="'.base_url('users/edit/'.$row['user_id']).'"> <i class="fa fa-eye"></i></a>
				<a title="Edit" class="update btn btn-xs btn-warning" href="'.base_url('users/edit/'.$row['user_id']).'"> <i class="fa fa-pencil-square-o"></i></a>
				<a title="Delete" class="delete btn btn-xs btn-danger" href='.base_url("users/delete/".$row['user_id']).' title="Delete" onclick="return confirm(\'Do you want to delete ?\')"> <i class="fa fa-trash-o"></i></a>'
			);
		}
		$records['data']=$data;
	...
	public function edit($id = 0){
		...
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == FALSE) {
					...
					$this->session->set_flashdata('errors', $data['errors']);
					redirect(base_url('users/user_edit/'.$id),'refresh');
			}
			else{
				$data = array(
					'admin_role_id' => $this->input->post('role'), 
